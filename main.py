...
def ten_medalists(output, filename, country, year, noc, sport):
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if element[6] == country or element[7] == noc and element[9] == year and element[14] in list_medals:
...
def main():
    if args.medals:
        ten_medalists(args.output, args.filename, args.country, args.year, args.noc, args.sport)
    if args.total:
        print(args.total)
        total(total)
    if args.overall:
